...
A poll's creator can also close it (`closePoll()`), permanently stopping further votes - proven by the same nullifier technique used for voters, so the creator's identity is exactly as private as everyone else's, and their own vote (if they cast one) stays unlinkable to their creator role.
...
- A compiled, tested Compact contract (`contract/`) - 11 tests covering tally correctness, double-vote rejection, identity non-linkage, and the creator-only `closePoll` flow.
- A TypeScript API layer (`api/`) wrapping deploy/join/castVote/closePoll.
- CLI deployment tooling (`cli/`) - deployed live to Midnight Preview.
- A React/Vite frontend (`frontend/`) - live poll display (no wallet needed), Lace-wallet-connected voting, and creator-only close-poll controls, deployed at [shadowpoll-frontend.vercel.app](https://shadowpoll-frontend.vercel.app).
...
- Multi-option polls (beyond yes/no) and poll expiry (time-based closing, not just creator-triggered).
